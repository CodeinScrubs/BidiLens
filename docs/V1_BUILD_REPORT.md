...
|---|---|---|
| `@bidilens/core` | Complete and tested | Unicode analysis, evidence, dual-offset isolation, security, streaming, properties; 95.15% lines |
...
| `@bidilens/web-component` | Complete and tested | SSR-safe registration, safe DOM construction, normal and self-contained CDN entries, real three-browser loading |
...
| Vitest within `check` | 15 files, 179 tests pass |
...
| `pnpm run action:check` | 181,907-byte bundle; Node 24 metadata/notices and unresolved-import checks pass; built artifact returns 0 for safe source and 2 for a strict high-risk control without mutation |
...
| `pnpm run release:check -- --allow-dirty` | development-tree build/pack/inspect/install/type/runtime/CLI consumer passes; exact examples extracted from all 12 tarballs execute. Commit the reviewed changes, then rerun without `--allow-dirty` to exercise the clean-worktree publication guard |
