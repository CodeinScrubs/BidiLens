...
| Swift Package/UIKit | Source and hosted compiler validation complete; physical-device/release validation pending | Swift Package, generated Unicode 17 ranges, copied 918-case corpus, core tests/example, UIKit `UILabel`/`UITextView`/`UITextField` adapters, independent physical alignment, and passing macOS/iOS compiler gates in [CI](https://github.com/CodeinScrubs/BidiLens/actions/runs/30349088058/job/90242135913). Physical iOS/VoiceOver evidence, sample app, dedicated SwiftUI view, and registry publication remain open |
