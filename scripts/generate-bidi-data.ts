...
async function main(): Promise<void> {
  ...
  if (check) {
    const [current, androidCurrent] = await Promise.all([
      readFile(OUTPUT_PATH, 'utf8'),
      readFile(ANDROID_OUTPUT_PATH, 'utf8')
    ]);
    if (current !== generated || androidCurrent !== androidGenerated) {
  ...
  console.log(`Generated web and Android Unicode tables from Unicode ${UNICODE_VERSION}.`);
}
